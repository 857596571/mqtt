...
public class Server {
    ...
    public static void main(String[] args) throws MqttException {
        ...
        server.message.setPayload("22222给客户端124推送的信息1232222".getBytes());
        server.publish(server.topic , server.message);

        server.message.setPayload("给客户端124推送的信息1232222".getBytes());
        server.publish(server.topic , server.message);

        server.message = new MqttMessage();
        server.message.setQos(2);
        server.message.setRetained(true);
        server.message.setPayload("给客户端125推送的信息abcccc".getBytes());
